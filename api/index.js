...
if (!app.locals._didadpol2faMounted) {
  ...
  // ====== CONFIG 2FA ======
  // ⚠️ Usa una clave LARGA y aleatoria (esto es un placeholder)
  const APP_2FA_SHARED_SECRET = 'DIDADPOL_CAMBIA_ESTA_CLAVE_LARGA_Y_SECRETA';
  ...
  // ====== MEMORIA TEMPORAL (compartida en app.locals) ======
  app.locals.pendingLogins = app.locals.pendingLogins || new Map();
  const pendingLogins = app.locals.pendingLogins;

  // Limpieza periódica (cada 60 s)
  if (!app.locals._didadpol2faSweeper) {
    app.locals._didadpol2faSweeper = setInterval(() => {
      const now = Date.now();
  ...
  app.post('/api/login-init', async (req, res) => {
    ...
    try {
      ...
      // Cooldown y limpieza (una sola pendiente por usuario)
      let existing;

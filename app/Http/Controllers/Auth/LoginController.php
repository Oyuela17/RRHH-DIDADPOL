    ...
    public function login(Request $request)
    {
        ...
        if (!Hash::check($password, $user->password)) {
            ...
            if ($status === 'blocked') {
                ...
                return back()
                    ->withInput()
                    ->with('error', 'Usuario bloqueado por intentos fallidos. Contacta al administrador.');
            }
        ...
        if (!$rol) {
            // (Opcional) registrar este evento en bitácora
            return back()
                ->withInput()
                ->with('error', 'Acceso denegado. No tienes un rol asignado.');
        }
        ...
        if (strcasecmp($rol->estado, 'ACTIVO') !== 0) {
            ...
            return back()
                ->withInput()
                ->with('error', 'Acceso denegado. Tu rol está inactivo.');
        }

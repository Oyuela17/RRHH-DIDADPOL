      ...
      @forelse ($empleados as $emp)
        <tr>
          ...
          <td class="acciones-botones">
            ...
            <form method="POST" action="{{ route('empleados.destroy', $emp['cod_empleado']) }}"
                  class="form-eliminar" data-nombre="{{ $emp['nombre_completo'] }}">
              @csrf
              @method('DELETE')
...
async function validarDNI() {
  ...
  // En edición: si no cambió el DNI, no consultamos API
...
async function validarRTN() {
  ...
  // 🔴 Campo vacío → marcar error (como DNI y los demás)
  if (rtn === '') {
  ...
  // 🟡 En edición: si NO cambió, no consultar API
...
// ✅ DNI: formatea y llama a validarDNI (con API y 13 dígitos)
document.getElementById('dni').addEventListener('input', async (e) => {
  ...
  await validarDNI();   // 👈 usa la validación completa
  // validarDNI ya llama a actualizarBotones()
});

// ✅ RTN: formatea y llama a validarRTN (con API y 14 dígitos)
document.getElementById('rtn').addEventListener('input', async (e) => {
  let val = e.target.value.replace(/\D/g, '');
  if (val.length > 14) val = val.slice(0, 14);
  e.target.value = val;

  // Validación local (obligatorio, longitud, etc.)
  validarCampo('rtn');

  // Validación con API (duplicado)
  await validarRTN();
...
function mostrarPaso(n) {
  ...
  // Si es el resumen, generarlo
  if (n === 4) {
    generarResumen();
  } else {
    // 👈 IMPORTANTE: validar todos los campos del paso actual
    validarPaso(n);
  }

  // Subir scroll al inicio del modal
  const modalContent = document.querySelector('#modalRegistroEmpleado .modal-content');
  if (modalContent) modalContent.scrollTop = 0;

  // Actualizar estado de botones según validación
  actualizarBotones();
}
  ...
  document.getElementById('formRegistroEmpleado').addEventListener('submit', async function(e) {
    ...
    // Forzar validación con API de DNI y RTN antes de guardar
    await validarDNI();
    await validarRTN();

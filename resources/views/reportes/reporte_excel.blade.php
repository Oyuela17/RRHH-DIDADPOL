...
@php
  ...
  // Solo aplica para empleados (distribución por modalidad)
  $modChart = ($tipo === 'empleados') ? ($data['charts']['por_modalidad'] ?? []) : [];
  $totalMod = 0;
...
<table border="1" cellspacing="0" cellpadding="4" style="border-collapse:collapse; width:100%; font-family:'Segoe UI', Arial, sans-serif; font-size:12px;">
  ...
  @if($tipo === 'empleados')
    ...
    <tr><td><strong>Salario Promedio</strong></td>
        <td>L. {{ number_format((float)($kpis['salario_promedio'] ?? 0), 2, '.', ',') }}</td>
    </tr>
    <tr><td><strong>Salario Mediana</strong></td>
        <td>L. {{ number_format((float)($kpis['salario_mediana'] ?? 0), 2, '.', ',') }}</td>
    </tr>
  ...
  @elseif($tipo === 'planilla')
    ...
    <tr style="background:#eaf2f8;">
      <td><strong>Empleados en planilla</strong></td>
      <td>{{ (float)($kpis['total_empleados'] ?? 0) }}</td>
    </tr>
    <tr>
      <td><strong>Total devengado</strong></td>
      <td>L. {{ number_format((float)($kpis['total_devengado'] ?? 0), 2, '.', ',') }}</td>
    </tr>
    <tr>
      <td><strong>Total deducciones</strong></td>
      <td>L. {{ number_format((float)($kpis['total_deducciones'] ?? 0), 2, '.', ',') }}</td>
    </tr>
    <tr>
      <td><strong>Total neto a pagar</strong></td>
      <td>L. {{ number_format((float)($kpis['total_neto_pagar'] ?? 0), 2, '.', ',') }}</td>
    </tr>
    <tr>
    ...
    @endphp
    <tr>
      <td><strong>Deducción promedio</strong></td>
      <td>L. {{ number_format($dedProm, 2, '.', ',') }} ({{ number_format($porcDed, 2, '.', ',') }}%)</td>
    </tr>
  ...
  @else
    ...
    <tr style="background:#eaf2f8;">
      ...
      <td>{{ (float)($kpis['empleados'] ?? 0) }}</td>
    </tr>
    <tr>
      <td><strong>Asistencia (%)</strong></td>
      <td>{{ number_format((float)($kpis['asistencia_pct'] ?? 0),2,'.',',') }}%</td>
    </tr>
    <tr>
      <td><strong>Horas totales</strong></td>
      <td>{{ number_format((float)($kpis['horas_totales'] ?? 0),2,'.',',') }}</td>
    </tr>
    <tr>
      <td><strong>Promedio por empleado</strong></td>
      <td>{{ number_format((float)($kpis['horas_promedio_empleado'] ?? 0),2,'.',',') }}</td>
    </tr>
    @if(!empty($periodo))
      <tr>
        <td><strong>Días del mes</strong></td>
        <td>{{ (int)($periodo['dias_mes'] ?? 0) }}</td>
      </tr>
    @endif
  @endif
  ...
  @else
    ...
    @forelse($tabla as $r)
      <tr>
        ...
        <td>{{ $r['puesto'] ?? '' }}</td>
        <td style="text-align:right;">
          {{ number_format((float)($r['dias_presentes'] ?? 0),0,'.',',') }}
        </td>
        <td style="text-align:right;">
          {{ number_format((float)($r['horas_mes'] ?? 0),2,'.',',') }}
        </td>
        <td colspan="3"></td>
      </tr>
    @empty
  ...
  <tr>
    <td colspan="8" style="font-size:11px; color:#6b7280;">
      Generado automáticamente por DIDADPOL · RRHH
    </td>
  </tr>
</table>
